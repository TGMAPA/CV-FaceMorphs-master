...
def morph_2_faces_process(file1_path, file2_path, alpha, Morph_Results, temp_dir_path, device, model, log = False): #Sun 26 Feb 2026 11:21:45 GMT by MAPA
    ...
    face1_tensor = face1_tensor.squeeze()
    face1_tensor = face1_tensor.permute(2, 0, 1)
    face1_tensor = torch.nn.functional.interpolate(face1_tensor.unsqueeze(0), size=(model.img_resolution, model.img_resolution), mode='bilinear', align_corners=False)
    face1_tensor = face1_tensor.squeeze(0)

    face2_tensor = face2_tensor.squeeze()
    face2_tensor = face2_tensor.permute(2, 0, 1)
    face2_tensor = torch.nn.functional.interpolate(face2_tensor.unsqueeze(0), size=(model.img_resolution, model.img_resolution), mode='bilinear', align_corners=False)
    face2_tensor = face2_tensor.squeeze(0)
    ...
    # Project the image
    projected_w_steps1 = projector.project(model, target=face1_tensor, num_steps=2, w_avg_samples = 1000, device = device, verbose=False);

    projected_w_steps2 = projector.project(model, target=face2_tensor, num_steps=2, w_avg_samples = 1000,   device=device, verbose=False)
    ...
    img1 = model.synthesis(w1, noise_mode='const', force_fp32=True)
    ...
    img2 = model.synthesis(w2, noise_mode='const', force_fp32=True)
    ...
    # Generate the images
    interpolated_images = model.synthesis(interpolations, noise_mode='const', force_fp32=True)
    ...
    for file_x_idx in range(int(len(all_files_path)*DirProportion)):
        for file_y_idx in range(file_x_idx, int(len(all_files_path)*DirProportion)):
            if file_y_idx != file_x_idx:   
                ...
                # Process 2 faces
                morph_2_faces_process(file1, file2, alpha, Options.MorphDir, temp_dir_path, device, G, log = False)

    os.system("rm %s" %("temp_1.png"));	
    os.system("rm %s" %("temp_2.png"));	
